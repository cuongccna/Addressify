...
export const vietnamProvinces = [
  ...
  { name: 'Hồ Chí Minh', aliases: ['ho chi minh', 'hcm', 'sai gon', 'saigon', 'tp hcm', 'tphcm'] },
...
export function processAddress(rawAddress: string): AddressData {
  ...
  const ward = extractWard(trimmed)
  ...
  return {
    ...
    normalizedAddress: isValid 
      ? `${finalWard ? finalWard + ', ' : ''}${finalDistrict}, ${finalProvince}`
      : rawAddress
  }
}

// Process multiple addresses from text input
